...
const runs=ref([]), calls=ref([]), failures=ref(null), vault=ref(null), versions=ref(null), versionPath=ref(''), error=ref('')
...
async function loadVersions(path){ try { versions.value=await api.get(`/artifacts/versions?path=${encodeURIComponent(path)}`) } catch(e){error.value=friendlyMessage(e)} }
async function rollback(v){ if (!versions.value || !confirm(`确认回退到版本 ${v.version}？`)) return; try { await api.post(`/artifacts/rollback?path=${encodeURIComponent(versions.value.path)}&version=${v.version}&confirm=true`); await loadVersions(versions.value.path) } catch(e){error.value=friendlyMessage(e)} }
onMounted(load)
...
<h2>失败清单</h2><el-empty v-if="!failures?.stages?.length&&!failures?.llm_calls?.length" description="暂无失败"/><pre v-else>{{ JSON.stringify(failures, null, 2) }}</pre><h2>产物版本回退</h2><el-space><el-input v-model="versionPath" placeholder="_noteagent/trees/树ID.md" style="width:320px"/><el-button @click="loadVersions(versionPath)" :disabled="!versionPath">查询版本</el-button></el-space><el-table v-if="versions" :data="versions.versions" stripe><el-table-column prop="version" label="版本" width="100"/><el-table-column prop="current" label="当前" width="100"/><el-table-column label="操作" width="120"><template #default="scope"><el-button v-if="!scope.row.current" size="small" @click="rollback(scope.row)">回退</el-button></template></el-table-column></el-table></div></template>
