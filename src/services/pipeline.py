...
from src.models.orm import Base, Association, Event, Extraction, LLMCall, Note
    ...
    def run(self) -> str:
        ...
        try:
            with bind_run(run.id):
                ...
                # 事件向量（DESIGN.md 4.2：事件内容 + 时间线索，供树重建检索）
                store.add_events([
                    {"id": f"{e.extraction_id}:{e.order_in_note}", "content": e.content, "time_clue": e.time_clue or "", "note_id": e.note_id}
                    for e in event_rows
                ])
